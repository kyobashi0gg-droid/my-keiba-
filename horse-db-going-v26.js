...
(() => {
  ...
  function goingClass(v) {
    ...
    if (/稍重|重|不良|稍|重馬場|道悪/.test(s)) return '道悪';
    if (/良/.test(s)) return '良';
    return '';
  }

  function raceGoing(race) {
    return goingClass(race?.going || race?.v3Going || race?.trackGoing || race?.condition || '');
  }

  function runGoing(run) {
    return goingClass(run?.going || run?.trackGoing || run?.condition || '');
  }
  ...
  function groupedGoingRows(runs = []) {
    ...
    return [...groups.values()].map(x => ({...x, summary:summarizeRuns(x.runs)}))
  ...
  async function decorateHorseModal() {
    ...
    if (!body || body.querySelector('.v26-going-zones')) return;
    const rows = groupedGoingRows(ctx.runs);
    if (!rows.length) return;

    const box = document.createElement('section');
    box.className = 'v26-going-zones';
    box.innerHTML = `
      <div class="v26-head"><strong>良馬場 / 道悪 33帯</strong><small>度外視候補は除外済み</small></div>
      <div class="v26-list">
      ${rows.map(x => `<div class="v26-row">
      ...
      <p>良と道悪を分離。レース当日の馬場が未設定なら、馬場状態では絞らず芝/ダート＋距離帯を優先します。</p>`;
  ...
  style.textContent = `
    ...
    .v26-head{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:8px}.v26-head strong{font-size:15px;color:#245f3f}.v26-head small{font-size:9px;color:#78857e}
